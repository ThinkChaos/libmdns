...
impl<'a> RRData<'a> {
    ...
    pub fn parse(typ: Type, rdata: &'a [u8], original: &'a [u8]) -> Result<RRData<'a>, Error> {
        match typ {
            ...
            Type::CNAME => Ok(RRData::CNAME(try!(Name::scan(rdata, original)).0)),
            Type::NS => Ok(RRData::NS(try!(Name::scan(rdata, original)).0)),
            Type::PTR => Ok(RRData::PTR(try!(Name::scan(rdata, original)).0)),
            Type::MX => {
                if rdata.len() < 3 {
                    return Err(Error::WrongRdataLength);
                }
